...
const App: React.FC = () => {
  ...
  const BeanDetails = ({ beanId }: { beanId: string }) => {
    ...
    // Custom optimal selection: Priority to manually marked, then fallback to highest rating
        ...
        <div className="px-6 space-y-8 flex-1 pb-40">
          ...
          <div className="space-y-4">
            ...
            {sortedShots.map(shot => (
              <div key={shot.id} className={`glass-card p-7 rounded-[42px] space-y-4 border-white/5 shadow-xl transition-all ${shot.isOptimal ? 'ring-2 ring-amber-500/50 bg-amber-500/[0.02]' : ''}`}>
                <div className="flex justify-between items-start">
                  <div className="flex flex-col gap-2">
                    ...
                    {shot.notes && <p className="text-[11px] text-stone-400 italic px-1 max-w-[200px] leading-tight line-clamp-2">{shot.notes}</p>}
                  </div>
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => toggleShotOptimal(shot.id, beanId)} 
                      className={`p-2 rounded-full transition-colors ${shot.isOptimal ? 'bg-amber-500 text-black' : 'bg-white/5 text-stone-700 hover:text-stone-400'}`}
  ...
  const AddShotForm = ({ beanId }: { beanId: string }) => {
    ...
    const [fd, setFd] = useState({ beanId, dose: last?.dose || 18, yield: last?.yield || 36, time: last?.time || 30, grindSetting: last?.grindSetting || '', rating: 7.0, notes: '', isOptimal: false });
    return (
      <div className="max-w-xl mx-auto px-6 pt-safe pb-safe min-h-[100dvh] flex flex-col fade-in">
        ...
        <form onSubmit={e => { e.preventDefault(); addShot(fd); }} className="space-y-8 flex-1 flex flex-col">
          ...
          <div className="glass-card p-10 rounded-[56px] space-y-8 shadow-xl">
            ...
            <div>
                <div className="flex justify-between items-center mb-6 px-2"><label className="text-[10px] text-stone-600 uppercase font-black">Success Rate</label><span className="text-amber-500 font-black text-xl">{fd.rating.toFixed(1)}/10</span></div>
          ...
          <div className="glass-card p-10 rounded-[56px] shadow-xl">
            <label className="text-[10px] text-stone-600 uppercase font-black mb-4 block">Extraction Notes</label>
            <textarea 
              placeholder="Observation: Channelling, puck screen used, turbo shot, etc..." 
              className="w-full bg-white/5 border border-white/5 rounded-[2rem] p-6 text-white outline-none focus:border-amber-500/30 transition-colors min-h-[100px] resize-none text-sm" 
              value={fd.notes} 
